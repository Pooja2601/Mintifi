...
const ThankYou = (props) => {
    ...
    const {payload, authObj, changeLoader, history, preFlightResp, loanPayload} = props;
    ...
    if (checkObject(payload) && checkObject(preFlightResp)) {
        ...
        if (window.location.host !== window.parent.location.host)
            ...
            if (checkObject(preFlightResp))
                window.setTimeout(() => window.location.href = `${payload.success_url}?drawdown_id=${preFlightResp.drawdown_id}`, 5000);
        }
