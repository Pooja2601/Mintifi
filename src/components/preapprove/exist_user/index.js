...
class Auth extends Component {
      ...
      .then(
        resp => {
          ...
          if (resp.error === Object(resp.error))
            ...
            if (resp.response.is_otp_verified) this.checkSummary();
          }
        },
        resp => {
          showAlert("net");
  ...
  _fetchUserInfo() {
    ...
    changeLoader(true);
    fetch(
      `${baseUrl}/users/user_summary?user_number=${authObj.mobile}&app_id=${app_id}`,
      {
        headers: { "Content-Type": "application/json", token: token }
      }
    )
  ...
  checkSummary = () => {
    ...
    changeLoader(true);
    fetch(
      `${baseUrl}/loans/loan_summary?user_number=${authObj.mobile}&anchor_transaction_id=${payload.anchor_transaction_id}&app_id=${app_id}`,
      {
        method: "GET",
        ...
        resp => {
          ...
          if (resp.response === Object(resp.response)) {
            ...
            if (response.loan_status === "new_user") {
              showAlert(
                "User doesn`t exist, redirecting you to the New User Portal...",
                "info"
              );
              setTimeout(
              ...
              showAlert(
                `Welcome Back ${response.user_name} , Let's create your Loan Application. Redirecting you in a while...`,
                "success"
              );
              ...
              showAlert(
                `Welcome Back ${response.user_name}, redirecting you to the dashboard...`,
                "success"
              );
              setTimeout(
                () => history.push(`${PUBLIC_URL}/exist/dashboard`),
                500
              );
            }
          }
